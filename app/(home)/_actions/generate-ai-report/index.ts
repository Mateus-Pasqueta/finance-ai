...
export const generateAiReport = async ({ month }: generateAiReportSchema) => {
  ...
  if (!hasPremiumPlan) {
    throw new Error("You need a premium plan");
  }
  if (!process.env.OPENAI_API_KEY) {
    await new Promise((resolve) => setTimeout(resolve, 1000));
    return DUMMY_REPORT;
  }
  const openaAi = new OpenAI({
  ...
  ${transactions
    ...
    .join(";")}`;
  const completion = await openaAi.chat.completions.create({
    model: "gpt-4o-mini",
    messages: [
      {
        role: "system",
  ...
  // pegar o relatorio gerado pelo ChatGPT e retornar para o usuário
